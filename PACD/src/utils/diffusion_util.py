...
def get_para_schedule(beta_schedule, beta_start, beta_end, num_diffusion_timestep, device):
    ...
    if beta_schedule == "quad":
        ...
        t = np.linspace(0, 1, num_diffusion_timestep, dtype=np.float64)
        alphas_bar = np.cos((t + 0.008) / 1.008 * np.pi / 2) ** 2
        alphas = np.concatenate([np.array([1.0]), alphas_bar[1:] / alphas_bar[:-1]])
        betas = 1.0 - alphas
        betas = beta_start + (beta_end - beta_start) * (betas / np.max(betas))
    else:
